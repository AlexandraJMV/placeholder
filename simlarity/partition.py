...
import pickle  # <--- ASEGÚRATE DE AGREGAR ESTO AL INICIO DEL ARCHIVO
...
def get_all_sim(args):
    ...
    print(f"Buscando matrices de similitud en: {args.sim_path}")

    for pair in list(comb):
        a, b = pair
        # Buscamos A vs B o B vs A (la similitud es simétrica)
        ...
        if os.path.exists(pickle1):
            print(f'   Cargando: {pickle1}')
            with open(pickle1, 'rb') as f:
                data = pickle.load(f)
        elif os.path.exists(pickle2):
            print(f'   Cargando: {pickle2}')
            with open(pickle2, 'rb') as f:
                data = pickle.load(f)
            is_transpose = True # Si cargamos B.A, tenemos que transponer para obtener A.B
        else:
            # Si no encuentra el archivo, lanzará este error.
            # ESTO PASARÁ AHORA MISMO (Ver Paso 2 abajo)
            raise FileNotFoundError(f"¡FALTA CKA! No se encontró ni {pickle1} ni {pickle2}. Necesitas calcular la similitud primero.")

        # Cargar en el diccionario
        if not is_transpose:
            all_sim_dict[f'{a}.{b}'] = data['sim']
            all_sim_dict[f'{b}.{a}'] = data['sim'].T
        else:
            all_sim_dict[f'{a}.{b}'] = data['sim'].T
            all_sim_dict[f'{b}.{a}'] = data['sim']
