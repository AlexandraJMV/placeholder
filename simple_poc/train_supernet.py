...
Changes required in eval_gt.py (no code delivered here):
  - Add --supernet_weights argument.
  - After building blueprint = SuperNetwork(...), load the supernet
    checkpoint with torch.load and blueprint.load_state_dict() BEFORE
    constructing SubNetworkExtractor.
  - This ensures GT fine-tuning starts from supernet weights, not raw
    ImageNet weights — Hypothesis A, which is the methodologically
    correct experiment for measuring τ.
"""
